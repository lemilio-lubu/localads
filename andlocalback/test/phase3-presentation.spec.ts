...
describe("Fase 3 - TransactionVerificationsController", () => {
  ...
  it("expone procesamiento/reintento OCR por receiptId", async () => {
    ...
    await controller.processOrRetryOcr("receipt-001");
  ...
  it("restringe la transición a revisión al rol administrador", () => {
    expect(Reflect.getMetadata(ROLES_KEY, TransactionVerificationsController.prototype.review)).toEqual(["ADMIN"]);
  });
