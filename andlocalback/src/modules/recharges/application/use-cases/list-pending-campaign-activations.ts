...
export type ListPendingCampaignActivationsQuery = Readonly<{ status?: ActivationRequestStatus; managerId?: string }>;
...
export class ListPendingCampaignActivations {
  ...
  execute(query: ListPendingCampaignActivationsQuery = {}): Promise<readonly CampaignActivationRequestView[]> {
    ...
    return this.persistence.list({ status, managerId: query.managerId?.trim() || undefined });
  }
}
