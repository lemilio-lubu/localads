...
export default function ClientDetailModal({ client, onClose }: Props) {
  ...
  return <ModalShell open labelledBy="client-detail-title" className={styles.modal} onClose={onClose}>
    ...
    <div className={styles.detailGrid}><section className={`${styles.panel} ${styles.cyclePanel}`}><h3>Ciclo más reciente</h3>{latest ? <ol className={styles.cycle}>{["solicitada", "aprobada", "en proceso", "completada"].map((step, index) => <li key={step} data-active={index < currentStep}><span>{index + 1}</span><div><strong>{step}</strong>{index === 0 && <small>{date(latest.createdAt)}</small>}</div></li>)}<li className={styles.payment}><small>estado del pago</small><strong>{paymentStatusLabel(latest.paymentStatus)}</strong></li></ol> : <p className={styles.empty}>Este cliente todavía no registra recargas.</p>}</section>
