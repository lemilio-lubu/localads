...
export default function LoginForm() {
  ...
  async function submitLogin(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setError(""); setPending(true); try { const user = await login(username.trim().toLowerCase(), password); router.push(user.role === "ADMIN" ? "/admin/clientes" : user.accountType === "POSTPAGO" ? "/flex" : "/prepago"); } catch (reason) { setError(reason instanceof Error ? reason.message : "Usuario o contraseña incorrectos"); setPending(false); } }
  function fillDemoAccount(type: "prepago" | "flex" | "admin") { setUsername(type); setPassword("1234"); setError(""); }
