...
export type MyTransactionFilters = { page?: number; limit?: number; search?: string; from?: string; to?: string };
export const getMyTransactions = (filters: MyTransactionFilters = {}) => {
  const query = new URLSearchParams({ page: String(filters.page ?? 1), limit: String(filters.limit ?? 20) });
  if (filters.search) query.set("search", filters.search);
  if (filters.from) query.set("from", `${filters.from}T00:00:00.000Z`);
  if (filters.to) query.set("to", `${filters.to}T23:59:59.999Z`);
  return api<PageResponse<TransactionListItem>>(`/me/transactions?${query.toString()}`);
};
