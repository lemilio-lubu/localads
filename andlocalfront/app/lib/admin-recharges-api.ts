...
export async function getAdminActivationRequests(status?: ActivationRequest["status"]): Promise<ActivationRequest[]> {
  if (status) return request<ActivationRequest[]>(`/admin/campaign-activation-requests${queryString({ status })}`);
  const [pending, review] = await Promise.all([getAdminActivationRequests("PENDING"), getAdminActivationRequests("IN_REVIEW")]);
